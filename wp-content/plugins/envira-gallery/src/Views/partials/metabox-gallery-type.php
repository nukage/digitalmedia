		...
		if ( count( $data['types'] ) > 1 ) {
			...
			if ( !empty( $option['type'] ) ) {
				...
				if ( $needs_update || !empty( $is_installed ) ) {
					...
					$upgrade_link 	= envira_get_upgrade_link();
					...
					<p>
						<a href="<?php echo $plugin_link; ?>" class="button button-primary button-x-large" title="<?php _e( 'Click Here to Upgrade', 'envira-gallery' ); ?>" target="_blank">
