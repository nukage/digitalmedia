	...
	var apps = {},
		...
		pixlrCallBack = function() {
			...
			if (pixlr) {
				...
				if (image[1].substr(0, 4) === 'http') {
					url = image[1];
					ext = url.replace(/.+\.([^.]+)jQuery/, 'jQuery1');
		...
		pixlrLoad = function(mode, base) {
			var self = this,
				...
				node = jQuery(base).children('img:first')
					.data('loading')()
					.data('resizeoff', function() {
						jQuery(window).off('resize.'+node.attr('id'));
				...
				dialog = jQuery(base).closest('.ui-dialog'),
				elfNode = fm.getUI(),
				uiToast = fm.getUI('toast'),
				container = jQuery('<iframe class="ui-front" allowtransparency="true">'),
				file = this.file,
				timeout = 15,
				...
				launch = function() {
					var src = 'https://pixlr.com/'+mode+'/?s=c',
						myurl = window.location.href.toString().replace(/#.*jQuery/, ''),
						opts = {};
				...
				errtm;
			jQuery(base).on('saveAsFail', launch);
			launch();
		},
		iframeClose = function(ifm) {
			var jQueryifm = jQuery(ifm),
				dfd = jQuery.Deferred().always(function() {
					jQueryifm.off('load', load);
				}),
				ab = 'about:blank',
				chk = function() {
	...
	return [
		{
			...
			load : function(base) {
				var self = this,
					...
					init = function(editor) {
						var jQuerybase = jQuery(base),
							bParent = jQuerybase.parent(),
							opts = self.confObj.opts,
							iconsPath = opts.iconsPath,
							tmpContainer = jQuery('<div class="tui-image-editor-container">').appendTo(bParent),
							tmpDiv = [
								jQuery('<div class="tui-image-editor-submenu"/>').appendTo(tmpContainer),
								jQuery('<div class="tui-image-editor-controls"/>').appendTo(tmpContainer)
							],
							iEditor = new editor(base, {
								includeUI: {
									loadImage: {
										path: jQuerybase.data('url'),
										name: self.file.name
									},
							...
							canvas = jQuerybase.find('canvas:first').get(0),
							zoom = function(v) {
							...
							zup = jQuery('<span class="ui-icon ui-icon-plusthick"/>').data('val', 10),
							zdown = jQuery('<span class="ui-icon ui-icon-minusthick"/>').data('val', -10),
							per = jQuery('<button/>').css('width', '4em').text('%').attr('title', '100%').data('val', 0),
							quty, qutyTm, zoomTm, zoomMore;

						tmpContainer.remove();
						jQuerybase.removeData('url').data('mime', self.file.mime);
						// jpeg quality controls
						if (self.file.mime === 'image/jpeg') {
							jQuerybase.data('quality', fm.storage('jpgQuality') || fm.option('jpgQuality'));
							quty = jQuery('<input type="number" class="ui-corner-all elfinder-resize-quality elfinder-tabstop"/>')
								...
								.val(jQuerybase.data('quality'));
							jQuery('<div class="ui-dialog-buttonset elfinder-edit-extras elfinder-edit-extras-quality"/>')
								.append(
									jQuery('<span>').html(fm.i18n('quality') + ' : '), quty, jQuery('<span/>')
								)
								.prependTo(jQuerybase.parent().next());
						} else if (self.file.mime === 'image/svg+xml') {
							jQuerybase.closest('.ui-dialog').trigger('changeType', {
								extention: 'png',
								mime : 'image/png',
								keepEditor: true
							});
						...
						// zoom scale controls
						jQuery('<div class="ui-dialog-buttonset elfinder-edit-extras"/>')
							.append(
								zdown, per, zup
							)
			...
			getContent : function(base) {
				var editor = this.editor,
					...
					quality = jQuerybase.data('quality');
				if (editor.instance) {
					if (jQuerybase.data('mime') === 'image/jpeg') {
					...
					return editor.instance.toDataURL({
						format: getExtention(jQuerybase.data('mime'), fm),
						quality: quality
					});
				}
			},
			save : function(base) {
				...
				if (hash) {
					...
					jQuerybase.data('mime', file.mime);
				}
			}
		},
		{
		...
		},
		{
			...
			init : function(id, file, dum, fm) {
				var orig = 'https://www.photopea.com',
					...
					spnr = jQuery('<div class="elfinder-edit-spinner elfinder-edit-photopea"/>')
			...
			load : function(base) {
				...
				if (self.initFail) {
					...
					jQuerybase.on('contentsloaded', function() {
		...
		},
		{
			...
			load : function(base) {
				var self = this,
					fm = this.fm,
					jQuerybase = jQuery(base),
					node = jQuerybase.children('img:first'),
					dialog = jQuerybase.closest('.ui-dialog'),
					elfNode = fm.getUI(),
					dfrd = jQuery.Deferred(),
					...
					init = function(onload) {
						...
						if (!container.length) {
							container = jQuery('<div id="elfinder-pixo-container" class="ui-front"/>').css({
								position: 'fixed',
								top: 0,
								right: 0,
								width: '100%',
						...
						dfrd.resolve(editor);
						jQuerybase.on('saveAsFail', launch);
						if (onload) {
							onload();
						}
					},
				...
				if (self.file.mime === 'image/jpeg') {
					...
					qBase = jQuery('<div class="ui-dialog-buttonset elfinder-edit-extras elfinder-edit-extras-quality"/>')
						.hide()
						.append(
							jQuery('<span>').html(fm.i18n('quality') + ' : '), quty, jQuery('<span/>')
						)
						.prependTo(jQuerybase.parent().next());
					jQuerybase.data('quty', quty);
				}
			...
			save : function(base) {
				var self = this,
					...
					q;
				if (base._canvas) {
					q = jQuerybase.data('quty')? Math.max(Math.min(jQuerybase.data('quty').val(), 100), 1) / 100 : void(0);
		...
		},
		{
			...
			load : function(base) {
				var self = this,
					...
					elfNode = fm.getUI(),
					dfrd = jQuery.Deferred(),
					container = jQuery('#elfinder-aviary-container'),
					init = function(onload) {
						...
						if (!container.length) {
							container = jQuery('<div id="elfinder-aviary-container" class="ui-front"/>').css({
								position: 'fixed',
								top: 0,
								right: 0,
								width: '100%',
						...
						opts = {
							...
							onSave: function(imageID, newURL) {
								...
								ext = newURL.replace(/.+\.([^.]+)jQuery/, 'jQuery1');
								if (node.data('ext') !== ext) {
									node.closest('.ui-dialog').trigger('changeType', {
										extention: ext,
										mime : ext2mime[ext]
			...
			// Convert content url to data uri scheme to save content
			save : function(base) {
				var node = jQuery(base).children('img:first');
			...
			load : function(textarea) {
				var self = this,
					...
					start = function() {
						...
						id = textarea.id + '_ace',
						ext = self.file.name.replace(/^.+\.([^.]+)|(.+)jQuery/, 'jQuery1jQuery2').toLowerCase(),
						// MIME/mode map
						mimeMode = {
							'text/x-php'			  : 'php',
						...
						editorBase = jQuery('<div id="'+id+'" style="width:100%; height:100%;"/>').text(ta.val()).insertBefore(ta.hide());
						...
						editor.jQueryblockScrolling = Infinity;
						editor.setOptions({
						...
						jQuery('<div class="ui-dialog-buttonset"/>').css('float', 'left')
						.append(
							jQuery('<button/>').html(self.fm.i18n('TextArea'))
							.button()
							.on('click', function(){
								if (ta.data('ace')) {
									ta.removeData('ace');
						...
						.append(
							...
							.on('click', function(){
								...
								jQuery('#ace_settingsmenu')
									...
									.parent().parent().appendTo(jQuery('#elfinder'));
							})
						)
			...
			load : function(textarea) {
				var fm = this.fm,
					...
					start = function(CodeMirror) {
						...
						if (! info && (m = self.file.name.match(/.+\.([^.]+)jQuery/))) {
				...
				var self = this,
					...
					init = function() {
						var base = jQuery(textarea).parent(),
							...
							h = base.height(),
							reg = /([&?]getfile=)[^&]+/,
							loc = self.confObj.managerUrl || window.location.href.replace(/#.*jQuery/, ''),
							name = 'ckeditor',
							opts;
						
						// make manager location
						if (reg.test(loc)) {
							loc = loc.replace(reg, 'jQuery1' + name);
						} else {
							loc += '?getfile=' + name;
						}
						// set base height
						...
						CKEDITOR.on('dialogDefinition', function(e) {
							...
							dlg.on('show', function(e) {
								fm.getUI().append(jQuery('.cke_dialog_background_cover')).append(this.getElement().jQuery);
							});
							dlg.on('hide', function(e) {
								jQuery('body:first').append(jQuery('.cke_dialog_background_cover')).append(this.getElement().jQuery);
							});
						});
					};
		...
		},
		{
			...
			init : function(id, file, data, fm) {
				var m = data.match(/^([\s\S]*<body[^>]*>)([\s\S]+)(<\/body>[\s\S]*)jQuery/i),
					header = '',
					body = '',
					footer ='';
				this.css({
		...
		},
		{
			...
			init : function(id, file, dum, fm) {
				var ta = this,
					ifm = jQuery(this).hide(),
					uiToast = fm.getUI('toast'),
					spnr = jQuery('<div class="elfinder-edit-spinner elfinder-edit-zohoeditor"/>')
						.html('<span class="elfinder-spinner-text">' + fm.i18n('nowLoading') + '</span><span class="elfinder-spinner"/>')
						.appendTo(ifm.parent()),
					cdata = function() {
						var data = '';
		...
		},
		{
			...
			init : function(id, file, dum, fm) {
				var ta = this,
					...
					btns = (function() {
						var btns = jQuery('<div/>').on('click', 'button', function() {
								var b = jQuery(this),
							...
							ul = jQuery('<ul/>'),
							oform = function(n, o) {
								var f = jQuery('<p/>').data('optkey', n).data('type', o.type),
									checked = '',
									disabled = '',
									nozip = false,
									opts, btn, elm;
								...
								f.append(jQuery('<span/>').text(fm.i18n(n) + ' : '));
								if (o.type === 'boolean') {
									...
									elm = jQuery('<select/>').append(jQuery('<option value=""/>').text('Select...'));
									jQuery.each(o['enum'], function(i, v) {
										elm.append(jQuery('<option value="'+v+'"/>').text(v));
									});
									f.append(elm);
								} else {
							...
							makeOption = function(o) {
								var elm = this,
									b = jQuery('<span class="elfinder-button-icon elfinder-button-icon-preference"/>').on('click', function() {
										f.toggle();
									}),
									f = jQuery('<div class="elfinder-edit-onlinconvert-options"/>').hide();
								if (o.options) {
						...
						jQuery.each(set.conv, function(t, c) {
							var cname = t.toLowerCase(),
								...
								type = jQuery('<div id="' + id + '" class="onlineconvert-category onlineconvert-category-'+cname+'"/>').data('cname', t),
								cext;
							jQuery.each(c, function(n, o) {
								...
								if (fm.uploadMimeCheck(confObj.ext2mime[ext], file.phash)) {
									type.append(jQuery('<div class="elfinder-edit-onlineconvert-button onlineconvert-'+nl+'"/>').on('makeoption', function(e, data) {
										var elm = jQuery(this);
										if (!elm.children('.elfinder-button-icon-preference').length) {
											makeOption.call(elm, data);
										}
									}).append(jQuery('<button/>').text(n).data('conv', n)));
								}
							});
						...
						if (set.useTabs) {
							...
							jQuery.each(set.conv, function(t) {
								...
								btns.append(jQuery('<fieldset class="onlineconvert-fieldset-' + tl + '"/>').append(jQuery('<legend/>').text(t)).append(btns.children('.onlineconvert-category-' + tl)));
							});
						}
						return btns;
					})(),
					...
					select = jQuery('<div/>')
						.append(
							btns,
							jQuery('<div class="elfinder-edit-onlineconvert-bottom-btn"/>').append(
								jQuery('<button/>')
					...
					spnr = jQuery('<div class="elfinder-edit-spinner elfinder-edit-onlineconvert"/>')
						.hide()
						.html('<span class="elfinder-spinner-text">' + fm.i18n('nowLoading') + '</span><span class="elfinder-spinner"/>')
						.appendTo(ifm.parent()),
					_url = null,
