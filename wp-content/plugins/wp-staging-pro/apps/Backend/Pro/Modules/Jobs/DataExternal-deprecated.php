 ...
 */
class DataExternal extends \WPStaging\Backend\Modules\Jobs\JobExecutable {

   /**
    * Staging Site database object
    * @var obj
    */
   private $stagingDb;

   /**
    * Production Database Object
    * @var \wpdb 
    */
...
//   }
   ...
   protected function isTable( $table ) {
      if( $this->productionDb->get_var( "SHOW TABLES LIKE '{$table}'" ) != $table ) {
   ...
   protected function step1() {
      ...
      $wpstg_existing_clones_beta = $this->productionDb->get_var( "SELECT option_value FROM {$this->productionDb->prefix}options WHERE option_name = 'wpstg_existing_clones_beta' " );
      ...
      $wpstg_existing_clones_beta_tmp_tbl = $this->productionDb->get_var( "SELECT option_value FROM {$optionsTableTmp} options WHERE option_name = 'wpstg_existing_clones_beta' " );
      ...
      if( false === $query ) {
         ...
         $this->returnException( "Data Crunching: Can not update table wpstg_existing_clones_beta in " . $optionsTableTmp . ' - db error: ' . $this->productionDb->last_error );
         return false;
      }

      // Delete wpstg_is_staging_site entry
      $resultDeleteIsStagingSite = $this->productionDb->query(
      ...
      if( false === $resultDeleteIsStagingSite ) {
         ...
         $this->returnException( "Data Crunching: Can not delete table row wpstg_is_staging_site from " . $optionsTableTmp . " - db error: " . $this->productionDb->last_error );
         return false;
      }

      // Get license data from live site and copy to tmp tables
      $wpstgLicense = $this->productionDb->get_var( "SELECT option_value FROM {$this->productionDb->prefix}options WHERE option_name = 'wpstg_license_key' " );

      $resultWpstgLicense = $this->productionDb->replace(
      ...
      // Get wpwstg settings data from live site and copy to tmp tables
      $wpstgSettings = $this->productionDb->get_var( "SELECT option_value FROM {$this->productionDb->prefix}options WHERE option_name = 'wpstg_settings' " );

      $resultWpstgSettings = $this->productionDb->replace(
              $this->tmpPrefix . 'options', array(
      ...
      if( false === $resultWpstgSettings ) {
         ...
         $this->returnException( 'Data Crunching: Error - Can not copy WP Staging settings from live site - db error: ' . $this->productionDb->last_error );
      }

      // Get wpwstg license status data from live site and copy to tmp tables
      $wpstg_license_status = $this->productionDb->get_var( "SELECT option_value FROM {$this->productionDb->prefix}options WHERE option_name = 'wpstg_license_status' " );

      $resultWpstg_license_status = $this->productionDb->replace(
              $this->tmpPrefix . 'options', array(
   ...
   protected function step2() {
      ...
      $this->log( "Data Crunching Step 2: Updating {$this->tmpPrefix}options table prefix to {$this->productionDb->prefix}. db error: {$this->productionDb->last_error}" );
      $this->debugLog( "Data Crunching Step 2: SQL - UPDATE {$this->tmpPrefix}options SET option_name = replace(option_name, {$this->options->prefix}, {$this->productionDb->prefix}) WHERE option_name LIKE {$this->options->prefix}_%" );

      $resultOptions = $this->productionDb->query(
              $this->productionDb->prepare(
                      "UPDATE IGNORE {$this->tmpPrefix}options SET option_name= replace(option_name, %s, %s) WHERE option_name LIKE %s", $this->options->prefix, $this->productionDb->prefix, $this->options->prefix . "_%"
              )
      );

      if( !$resultOptions ) {
         $this->log( "Data Crunching Step 2: Failed to update {$this->tmpPrefix}options with table prefixes. DB Error: {$this->productionDb->last_error}" );
         $this->returnException( "Data Crunching Step 2: Failed to update {$this->tmpPrefix}options with table prefixes {$this->productionDb->prefix}. DB Error: {$this->productionDb->last_error}" );
         return false;
      }
   ...
   protected function step3() {
      ...
      $this->log( "Data Crunching Step 3: Updating {$this->tmpPrefix}usermeta db prefix to {$this->productionDb->prefix} {$this->productionDb->last_error}" );
      ...
      $resultMetaKeys = $this->productionDb->query(
              $this->productionDb->prepare(
                      "UPDATE {$this->tmpPrefix}usermeta SET meta_key = replace(meta_key, %s, %s) WHERE meta_key LIKE %s", $this->options->prefix, $this->productionDb->prefix, $this->options->prefix . "_%"
              )
      );

      if( !$resultMetaKeys ) {
         $this->log( "Data Crunching Step 3: SQL - UPDATE {$this->tmpPrefix}usermeta SET meta_key = replace(meta_key, {$this->options->prefix}, {$this->productionDb->prefix}) WHERE meta_key LIKE {$this->options->prefix}_%" );
         $this->log( "Data Crunching Step 3: Failed to update usermeta meta_key database table prefixes {$this->productionDb->last_error}" );
         $this->returnException( "Data Crunching Step 3: Failed to update {$this->tmpPrefix}usermeta meta_key database table prefixes {$this->productionDb->last_error}" );
         return false;
      }
   ...
   protected function step4() {
      ...
      $activePlugins = $this->productionDb->get_var( "SELECT option_value FROM {$this->tmpPrefix}options WHERE option_name = 'active_plugins' " );
      ...
      if( false === $resultActivePlugins ) {
         ...
         $this->returnException( "Data Crunching Step 4: Can not update table active_plugins in {$this->tmpPrefix}options - db error: " . $this->productionDb->last_error );
         return false;
      }
   ...
   protected function step5() {
      ...
      $sessionToken = $this->productionDb->get_var( "SELECT meta_value FROM {$this->productionDb->prefix}usermeta WHERE meta_key = 'session_tokens' AND user_id = '{$userId}'" );

      $sessionToken = unserialize( $sessionToken );

      if( !$sessionToken ) {
         $this->log( "Data Crunching Step 5: Can not get session token of current user from {$this->productionDb->prefix}usermeta ", Logger::TYPE_WARNING );
         //$this->returnException("Data Crunching Step 5: Can not get session token from {$this->productionDb->prefix}usermeta");
      }
      ...
      if( false === $resultSessionToken ) {
         ...
         //$this->returnException("Data Crunching Step 5: Can not update row session_tokens in {$this->tmpPrefix}usermeta - db error: " . $this->productionDb->last_error);
         return false;
      }
   ...
   protected function step6() {
      ...
      $permalink = $this->productionDb->get_var( "SELECT option_value FROM {$this->productionDb->prefix}options WHERE option_name = 'permalink_structure' " );

      //$permalink = unserialize($permalink);

      if( !$permalink ) {
         $this->log( "Data Crunching Step 6: Can not get permalink_structure from {$this->productionDb->prefix}options " );
         //$this->returnException("Data Crunching Step 6: Can not get permalink_structure from {$this->productionDb->prefix}options");
         $permalink = '/%postname%/';
      }
      // Update permalink_structure
      ...
      if( false === $resultPermalink ) {
         ...
         $this->returnException( "Data Crunching Step 6: Can not update row permalink_structure in {$this->tmpPrefix}options - db error: " . $this->productionDb->last_error );
         return false;
      }
   ...
   protected function step7() {
      ...
      $siteUrl = $this->productionDb->get_var( "SELECT option_value FROM {$this->productionDb->prefix}options WHERE option_name = 'siteurl' " );

      if( !$siteUrl ) {
         $this->log( "Data Crunching Step 7: Can not get siteurl from {$this->productionDb->prefix}options " );
         $this->returnException( "Data Crunching Step 7: Can not find siteurl in {$this->productionDb->prefix}options" );
      }
      // Update siteurl
      $resultSiteUrl = $this->productionDb->query(
      ...
      if( false === $resultSiteUrl ) {
         ...
         $this->returnException( "Data Crunching Step 7: Can not update row siteurl in {$this->tmpPrefix}options - db error: " . $this->productionDb->last_error );
         return false;
      }

      // Get home value from live site options table
      $home = $this->productionDb->get_var( "SELECT option_value FROM {$this->productionDb->prefix}options WHERE option_name = 'home' " );

      if( !$home ) {
         $this->log( "Data Crunching Step 7: Can not get home or is empty from {$this->productionDb->prefix}options " );
         //$this->returnException( "Data Crunching Step 7: Can not find home or is empty in {$this->productionDb->prefix}options" );
      }
      // Update home
      $resultHome = $this->productionDb->query(
      ...
      if( false === $resultHome ) {
         ...
         $this->returnException( "Data Crunching Step 7: Can not update row home in {$this->tmpPrefix}options - db error: " . $this->productionDb->last_error );
         return false;
      }
   ...
   protected function step8() {
      ...
      $select = $this->productionDb->get_var( "SELECT option_value FROM {$this->productionDb->prefix}options WHERE option_name = 'wpstgpro_version' " );

      if( !$select ) {
         $this->log( "Data Crunching Step 8: Can not get wpstgpro_version from {$this->productionDb->prefix}options " );
         $this->returnException( "Data Crunching Step 8: Can not find wpstgpro_version in {$this->productionDb->prefix}options" );
      }
      ...
      if( false === $update ) {
         ...
         $this->returnException( "Data Crunching Step 8: Can not update row wpstgpro_version in {$this->tmpPrefix}options - db error: " . $this->productionDb->last_error );
         return false;
      }
   ...
   protected function step9() {
      ...
      $result = $this->productionDb->get_var( "SELECT option_value FROM {$this->productionDb->prefix}options WHERE option_name = 'blog_public' " );

      if( !$result ) {
         $this->log( "Data Crunching Step 9: Can not find blog_public in {$this->productionDb->prefix}options ", Logger::TYPE_WARNING );
         //$this->returnException("Data Crunching Step 9: Can not get blog_public in {$this->productionDb->prefix}options");
      }
      // Update blog_public
      $update = $this->productionDb->query(
      ...
      if( false === $update ) {
         ...
         //$this->returnException("Data Crunching Step 9: Can not update blog_public in {$this->tmpPrefix}options - db error: " . $this->productionDb->last_error);
         return true;
      }
   ...
   protected function step10() {
      $this->log( "Data Crunching Step10: Preserve Data in " . $this->productionDb->prefix . "_options" );

      // Skip - Table does not exist
      if( false === $this->isTable( $this->options->prefix . 'options' ) ) {
         return true;
      }
      ...
      $preserved_option_names = array('siteurl', 'home');
      ...
      // Get preserved data from wp_options tables
      $table = $this->productionDb->prefix . 'options';
      $preserved_options_data[$this->tmpPrefix . 'options'] = $this->productionDb->get_results(
              sprintf(
      ...
      $this->log( "Data Crunching Step10: Successfull!" );
      return true;
   }
   ...
   private function executeSql( $sqlbatch ) {
      ...
      foreach ( $queries as $query ) {
         if( false === $this->productionDb->query( $query ) ) {
