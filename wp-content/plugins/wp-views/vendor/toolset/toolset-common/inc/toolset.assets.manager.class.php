...
class Toolset_Assets_Manager {
	...
	/**
	 * Register the shared icons and UI OTGS assets.
	 *
	 * Registration is managed by each of the dependencies themselves.
	 * After registration is completed at init:10, those assets are available
	 * to be declared as dependencies.
	 *
	 * @return void
	 */
	protected function initialize_vendor_assets() {
		/*
		 * Declare the vendor directory root URL, used by the loader scripts.
		 *
		 * IMPORTANT: DO NOT REMOVE this variable o the whole set of assets will not be registered.
		 */
		$vendor_root_url = dirname( dirname( $this->assets_url ) );
		if ( file_exists( TOOLSET_COMMON_VENDOR_PATH .  '/otgs/icons/loader.php' ) ) {
			require_once( TOOLSET_COMMON_VENDOR_PATH .  '/otgs/icons/loader.php' );
		}
		if ( file_exists( TOOLSET_COMMON_VENDOR_PATH .  '/otgs/ui/loader.php' ) ) {
			require_once( TOOLSET_COMMON_VENDOR_PATH .  '/otgs/ui/loader.php' );
		}
	}
